    <!-- Delete Customer -->
    <div class="modal fade" id="del_<?php echo $cqrow['deliveryId']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <center><h4 class="modal-title" id="myModalLabel">Delete / Deactivate Delivery</h4></center>
                </div>
                <div class="modal-body">
				<div class="container-fluid">
					<?php
						$a=mysqli_query($conn,"Select tbldelivery.*, tblperson.personId, CONCAT(tblperson.firstName,' ',tblperson.middleName,' ',tblperson.lastName,' ') AS 'Fullname' from tbldelivery inner join tblperson on tblperson.personId=tbldelivery.personId where deliveryId ='".$cqrow['deliveryId']."'");
						$b=mysqli_fetch_array($a);
					?>
                    <h5><center>Delivery Name: <strong><?php echo ucwords($b['Fullname']); ?></strong></center></h5>
					<form role="form" method="POST" action="deleteadelivery.php<?php echo '?id='.$cqrow['deliveryId']; ?>">
                </div>                 
				</div>
...
<!-- Edit Employee -->
    <div class="modal fade" id="edit_<?php echo $cqrow['deliveryId']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <center><h4 class="modal-title" id="myModalLabel">Edit Delivery</h4></center>
                </div>
                <div class="modal-body">
				<div class="container-fluid">
					<?php
						$a=mysqli_query($conn,"SELECT tbldelivery.*,tblperson.*,tblaccount.accountUserName,tblaccount.accountPassword ,tblbarangay.barangayId, tblbarangay.description as brgy, tblprovince.description as province,tbluserstatus.description as status
                            from tblperson 
                            inner join tblaccount on tblaccount.personId=tblperson.personId 
                            INNER join tbldelivery on tbldelivery.personId=tblperson.personId 
                            ...
                            where tbldelivery.deliveryId='".$cqrow['deliveryId']."'"); 
						$b=mysqli_fetch_array($a);
					?>
					<div style="height:10px;"></div>
                    <form role="form" method="POST" action="edit_delivery.php<?php echo '?id='.$cqrow['deliveryId']; ?>">
                        <div class="form-group input-group">
                            <span class="input-group-addon" style="width:120px;">Cutomer Id:</span>
                            <input disabled type="text" style="width:400px; text-transform:capitalize;" value="<?php echo ucwords($b['deliveryId']); ?>" class="form-control" name="deliveryId">
                        </div>
                            <div style="height:10px;"></div>
                        <div class="form-group input-group">
                          ...
                          <div class="form-group input-group">
                            <span style="width:120px;" class="input-group-addon">Warehouse: </span>
                             ...
                             <?php
                                    ...
                                    $cat=mysqli_query($conn,"select * from tblwarehouse");
                                    while($catrow=mysqli_fetch_array($cat)){

                                        if($b['warehouseId'] == $catrow['warehouseId']){
                                             ?>
                                                 <option selected value="<?php echo $catrow['warehouseId']; ?>"><?php echo $catrow['warehouseName']; ?></option>
                                             <?php
                                        }else{
                                            ?>
                                                <option value="<?php echo $catrow['warehouseId']; ?>"><?php echo $catrow['warehouseName']; ?></option>
                                            <?php
                                        }
                                    }
                                ?>
