    <!-- Delete Customer -->
    <div class="modal fade" id="del_<?php echo $cqrow['employeeId']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <center><h4 class="modal-title" id="myModalLabel">Delete / Deactivate Employee</h4></center>
                </div>
                <div class="modal-body">
				<div class="container-fluid">
					<?php
						$a=mysqli_query($conn,"Select tblemployee.*, tblperson.personId, CONCAT(tblperson.firstName,' ',tblperson.middleName,' ',tblperson.lastName,' ') AS 'Fullname' from tblemployee inner join tblperson on tblperson.personId=tblemployee.personId where employeeId ='".$cqrow['employeeId']."'");
						$b=mysqli_fetch_array($a);
					?>
                    <h5><center>Employee Name: <strong><?php echo ucwords($b['Fullname']); ?></strong></center></h5>
					<form role="form" method="POST" action="deleteemployee.php<?php echo '?id='.$cqrow['employeeId']; ?>">
                </div>                 
				</div>
...
<!-- Edit Employee -->
    <div class="modal fade" id="edit_<?php echo $cqrow['employeeId']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				...
				<div class="container-fluid">
					<?php
						$a=mysqli_query($conn,"SELECT tblemployee.*,tblperson.*,tblaccount.accountUserName,tblaccount.accountPassword ,tblbarangay.barangayId, tblbarangay.description as brgy, tblprovince.description as province,tbluserstatus.description as status, tblwarehouse.warehouseName
                            from tblperson 
                            inner join tblaccount on tblaccount.personId=tblperson.personId 
                            INNER join tblemployee on tblemployee.personId=tblperson.personId 
                            ...
                            where tblemployee.employeeId='".$cqrow['employeeId']."'"); 
						$b=mysqli_fetch_array($a);
					?>
					<div style="height:10px;"></div>
                    <form role="form" method="POST" action="edit_employee.php<?php echo '?id='.$cqrow['employeeId']; ?>">
                        <div class="form-group input-group">
                            <span class="input-group-addon" style="width:120px;">Cutomer Id:</span>
                            <input disabled type="text" style="width:400px; text-transform:capitalize;" value="<?php echo ucwords($b['employeeId']); ?>" class="form-control" name="employeeId">
                        </div>
                            <div style="height:10px;"></div>
                        <div class="form-group input-group">
                          ...
                          <div class="form-group input-group">
                            <span style="width:120px;" class="input-group-addon">Warehouse: </span>
                             ...
                             <?php
                                    ...
                                    while($catrow=mysqli_fetch_array($sup)){
