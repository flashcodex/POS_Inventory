...
<!-- Edit Profile -->
    ...
    <?php
        $cq=mysqli_query($conn,"select tbldelivery.*,tblperson.*,tblaccount.accountUserName,tblaccount.accountPassword ,tblbarangay.barangayId, tblbarangay.description as brgy, tblprovince.description as province,tbluserstatus.description as status from tblperson inner join tblaccount on tblaccount.personId=tblperson.personId INNER join tbldelivery on tbldelivery.personId=tblperson.personId inner join tblprovince on tblprovince.provinceId=tblperson.provinceId INNER join tblbarangay on tblbarangay.barangayId=tblperson.brangayId inner join tbluserstatus on tbluserstatus.statusId=tblaccount.accountStatus where tblperson.personId=$personId");
